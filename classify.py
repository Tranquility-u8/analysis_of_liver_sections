...
def getGLCMFeature(gray_image, contour):
    ...
    # 使用cv2.drawContours在掩模上绘制实心轮廓
    cv2.drawContours(mask, [contour], -1, 255, thickness=cv2.FILLED)

    # 应用掩模，只保留轮廓内的区域
    gray_region = cv2.bitwise_and(gray_image, gray_image, mask=mask)

    # 计算GLCM
    glcm = feature.graycomatrix(gray_region, [1], [0, np.pi / 4, np.pi / 2, 3 * np.pi / 4], 256, symmetric=True,
                                normed=True)
...
def getRegionClass(area, ratio, solidity, hu_moments, homogeneity):
    ...
    if area > 180000 and solidity + hu_moments[0] < 0.92:
